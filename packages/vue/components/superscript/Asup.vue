...
sup(
  ...
  :class="[ci ? cspacex : '', cipos === 'right' ? 'flex-row-reverse space-x-reverse' : '', cc]"
)
  slot
...
<script lang="ts">
export default {
	name: "Asup", // name field must be specified for plugin (global component registration) to work
	inheritAttrs: false,
};
</script>

<script setup lang="ts">
import type { IASupProps } from "@agufaui/theme";
...
const props = withDefaults(defineProps<IASupProps>(), {
	// #region props
	spacex: "space-x-1.5",
	// #endregion props
});
...
const computedProperties = getComputedFromProps<IASupProps>(props, CASupName, config);
