...
export function getUsedInterfacesFromAst(ast: Program) {
	...
	const addInterface = (node: Node) => {
		if (
			...
			if (
				propsTypeDefinition.type === "TSTypeReference" &&
				propsTypeDefinition.typeName.type === "Identifier"
			) {
	...
	return ast.body
		.map((node) => {
			...
			if (
				...
				node.declaration &&
				isTSTypes(node.declaration, isInternal)
			)
...
export async function extractTypesFromSource(
	...
	const extractFromPosition = (start: number | null | undefined, end: number | null | undefined) =>
	...
	const extractTypesFromInterface = (node: TSInterfaceDeclaration) => {
		const { extendInterfaceName, interfaceBodyStart, isProperty } =
			metaDataMap.get(node.id.name) ?? {};
	...
	const extractTypesFromEnum = (node: TSEnumDeclaration) => {
		...
		extractedTypes.set(
			enumName,
			`type ${enumName} = ${[...enumTypes].join(" | ") || "number | string"};`
		);
	};
